    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 relative overflow-hidden font-sans select-none">
      ...
      <div className="w-full max-w-md bg-slate-900/40 backdrop-blur-xl border border-slate-800/80 p-8 rounded-3xl shadow-2xl shadow-slate-950/80 relative z-10 transition hover:border-slate-700/60 duration-300">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-xs text-slate-400 mt-1.5">Regístrate como doctor o paciente para comenzar el seguimiento.</p>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Tipo de Cuenta</label>
            <div className="grid grid-cols-2 gap-3 mt-1">
              <button
                type="button"
                onClick={() => setRole('USER_PACIENTE')}
              ...
              <button
                type="button"
                onClick={() => setRole('ADMIN_NUTRIOLOGO')}
                className={`py-2 text-xs font-bold rounded-xl border transition ${
          ...
          {/* Mostrar selector de nutriólogos únicamente si el rol es paciente */}
          {role === 'USER_PACIENTE' && nutriologos.length > 0 && (
            <div>
